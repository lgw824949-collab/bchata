...
for (const loc of data || []) {
  const { region } = classifyVenueLocation(loc);
  counts[region] = (counts[region] || 0) + 1;
}
console.log('counts', counts);

const seoulWrong = (data || []).filter((loc) => {
  const { region } = classifyVenueLocation(loc);
  return region === '서울' && !`${loc.address || ''}`.includes('서울');
});
console.log('서울로 잘못 분류', seoulWrong.map((x) => x.name));
