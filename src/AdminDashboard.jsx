  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (category === 'social') {
        ...
        query = supabase.from(table).select('*').eq('status', activeTab);
      }
      ...
      if (category === 'social') {
        if (newStatus === 'active') {
          ...
          const { error: insError } = await supabase.from('parties').insert([{
            title: item.title, location_id: locData?.id, address: item.address, fee: item.fee,
            ...
            <div style={{ display: 'flex', gap: '16px' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: 'flex', gap: '8px', marginTop: '16px' }}>
                  {category !== 'live' && (
                    <>
                      <button onClick={() => updateStatus(item, 'active')} style={{ flex: 1, padding: '10px', borderRadius: '10px', border: 'none', background: '#E8F5E9', color: '#2E7D32' }}><Check size={18} /></button>
                      <button onClick={() => updateStatus(item, 'pending')} style={{ flex: 1, padding: '10px', borderRadius: '10px', border: 'none', background: '#FFF8E1', color: '#F59E0B' }}><Clock size={18} /></button>
                      <button onClick={() => updateStatus(item, 'rejected')} style={{ flex: 1, padding: '10px', borderRadius: '10px', border: 'none', background: '#FFEBEE', color: '#C62828' }}><X size={18} /></button>
                    </>
                  )}
                  <button onClick={() => deleteItem(item.id)} style={{ flex: category === 'live' ? 1 : 'none', padding: '10px', borderRadius: '10px', border: 'none', background: '#F5F5F5', color: '#666' }}><Trash2 size={18} /></button>
                </div>
              </div>
            </div>
          </div>
